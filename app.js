...
(function () {
  ...
  function publicCaregiverCardMarkup(profile) {
    ...
    const competency = caregiverCompetencyRadarMarkup(profile.competencyAverages, profile.competencyReviewCount, true)
      || `<div class="caregiver-competency-empty compact"><strong>전문 역량 후기 준비 중</strong><span>첫 후기가 등록되면 6개 역량 평균이 표시됩니다.</span></div>`;
    return `<article class="public-caregiver-profile-card ${profile.featured ? "featured" : ""}"><div class="caregiver-public-card-top"><div class="caregiver-public-photo">${caregiverPublicPortraitMarkup(profile)}</div><div class="caregiver-public-card-summary"><div class="caregiver-public-name"><div><small>${profile.featured ? "FEATURED CARE PROFESSIONAL" : "CARE PROFESSIONAL"}</small><h3>${escapeHtml(profile.displayName)}</h3></div>${caregiverPublicRatingMarkup(profile, true)}</div><p class="caregiver-public-headline">${escapeHtml(profile.headline)}</p>${caregiverServiceBadgesMarkup(profile)}</div></div><div class="caregiver-public-card-copy"><div class="caregiver-public-facts"><span>경력 ${Number(profile.careerYears || 0).toLocaleString("ko-KR", { maximumFractionDigits: 1 })}년</span>${profile.serviceArea ? `<span>${escapeHtml(profile.serviceArea)}</span>` : ""}</div><div class="caregiver-public-tags">${specialties.map((item) => `<span>${escapeHtml(item)}</span>`).join("")}</div><div class="caregiver-public-radar-slot">${competency}</div><p class="caregiver-public-bio">${escapeHtml(profile.biography || "가족의 돌봄 필요를 세심하게 살피는 ProMoms 관리사입니다.")}</p><button type="button" class="secondary-button" data-public-caregiver-detail="${profile.caregiverId}">프로필·후기 보기</button></div></article>`;
  }
