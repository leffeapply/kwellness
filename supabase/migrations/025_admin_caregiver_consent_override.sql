-- Administrators may grant caregiver workspace access immediately. This is an
-- administrative access override, not a representation of member consent.
...
declare
  ...
  caregiver_consent_override boolean := false;
begin
  ...
  if exists (
    select 1
    from unnest(normalized_roles) as assigned(role)
    where assigned.role not in ('OWNER', 'ADMIN', 'CARE_MANAGER', 'CAREGIVER', 'CLIENT', 'RETAIL_STAFF')
  ) then
  ...
  -- Missing consent is recorded as an administrative override. No consent row
  -- is inserted because the member did not personally grant that consent.
  if adding_caregiver then
    caregiver_consent_override := exists (
      select 1
      from (
  ...
  if adding_caregiver then
    insert into public.caregiver_applications (
      ...
      now(),
      case
        when caregiver_consent_override then '관리자 예외 승인 · 회원 사전 약관 동의 없음'
        else '관리자 직접 승인'
      end
    )
  ...
  select p_user_id, assigned.role
  from unnest(normalized_roles) as assigned(role);
  ...
  values (
    auth.uid(),
    'CONFIGURE_MEMBER_ACCESS_ROLES',
    'user_roles',
    p_user_id,
    jsonb_build_object(
      'old_roles', existing_roles,
      'new_roles', normalized_roles,
      'caregiver_consent_override', caregiver_consent_override,
      'override_granted_by', case when caregiver_consent_override then auth.uid() else null end
    )
  );
end;
$$;
...
comment on function public.admin_set_member_access_roles(uuid, public.app_role[]) is
  'Configures combined member workspaces. Admin caregiver grants may bypass missing member consent without creating consent records, and are audited.';
