-- Six-axis caregiver competency reviews.
-- Existing reviews remain valid overall ratings. Competency aggregates include
-- only reviews that contain all six independently submitted scores.
...
alter table public.caregiver_reviews
  drop constraint if exists caregiver_reviews_competency_scores_check,
  add constraint caregiver_reviews_competency_scores_check check (
    num_nonnulls(
    ...
    and (meal_preparation_score is null or meal_preparation_score between 1 and 5)
    and (attentiveness_score is null or attentiveness_score between 1 and 5)
    and (punctuality_score is null or punctuality_score between 1 and 5)
    and (professionalism_score is null or professionalism_score between 1 and 5)
    and (communication_score is null or communication_score between 1 and 5)
    and (hygiene_safety_score is null or hygiene_safety_score between 1 and 5)
  );

alter table public.caregiver_historical_reviews
  drop constraint if exists caregiver_historical_reviews_competency_scores_check,
  add constraint caregiver_historical_reviews_competency_scores_check check (
    num_nonnulls(
      meal_preparation_score,
      attentiveness_score,
      punctuality_score,
...
drop function if exists public.submit_caregiver_review(uuid, integer, text[], text, boolean);
  ...
  left join lateral (
    ...
    from (
      select
        ...
        review.meal_preparation_score,
        review.attentiveness_score,
        review.punctuality_score,
        review.professionalism_score,
        review.communication_score,
        review.hygiene_safety_score
      from public.caregiver_reviews review
      ...
      select
        ...
        historical.meal_preparation_score,
        historical.attentiveness_score,
        historical.punctuality_score,
        historical.professionalism_score,
        historical.communication_score,
        historical.hygiene_safety_score
...
revoke all on function public.submit_caregiver_review(
  uuid, integer, text[], text, boolean, jsonb
) from public, anon;
grant execute on function public.submit_caregiver_review(
  uuid, integer, text[], text, boolean, jsonb
) to authenticated;
...
comment on column public.caregiver_reviews.communication_score is
  'One of six customer-submitted caregiver competency scores; all six are stored together.';
comment on column public.caregiver_historical_reviews.communication_score is
  'One of six evidence-backed external-review caregiver competency scores; all six are stored together.';
comment on function public.public_caregiver_directory() is
  'Sanitized caregiver directory with separate overall rating and six-axis competency averages; legacy reviews remain in overall rating only.';
